...
const useCart = () => {
  ...
  const updateCart = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<Cart> }) => apiClient.update(id, data),
    onMutate: ({ id, data }) => {
      queryClient.cancelQueries({ queryKey: ['cart'] })

      const previousCarts = queryClient.getQueryData<Cart[]>(['cart'])
      const optimisticUpdate = previousCarts?.map((cartItem) => cartItem.id === id ? { ...cartItem, ...data } : cartItem)
  ...
  const deleteCart = useMutation({
    ...
    onMutate: (id) => {
      ...
      const previousCarts = queryClient.getQueryData<Cart[]>(['cart'])
      const optimisticUpdate = previousCarts?.filter((cartItem) => cartItem.id !== id)
